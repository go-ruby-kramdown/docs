...
`go-ruby-kramdown/kramdown` reimplements Ruby's `kramdown` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-kramdown/kramdown` directly, with no Ruby runtime;
